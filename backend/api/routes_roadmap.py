...
from models.database_models import Resume, JobDescription, Roadmap, Progress
...
def get_roadmap(user_id: str):
    ...
    db = SessionLocal()
    try:
        user_data = db.query(Resume).filter(Resume.user_id == user_id).first()
        job_data = db.query(JobDescription).filter(JobDescription.user_id == user_id).first()
